    ...
    <nav className='fixed top-0 left-0 w-full bg-black bg-opacity-75 backdrop-blur rounded-b-2xl text-white'>
      <div className='flex flex-row justify-between items-center max-w-screen-xl mx-auto px-12 py-6'>
        ...
        <ul className='hidden md:flex gap-8 text-white font-bold'>
          <li className={`p-2 ${location.pathname === '/work' ? 'underline underline-offset-4 decoration-2' : ''}`}>
            <Link to="/work" className='hover:duration-400 ease-out hover:underline hover:underline-offset-4 hover:decoration-2'>WORK</Link>
          </li>
          <li className={`p-2 ${location.pathname === '/aboutme' ? 'underline underline-offset-4 decoration-2' : ''}`}>
            <Link to="/aboutme" className='hover:duration-400 ease-out hover:underline hover:underline-offset-4 hover:decoration-2'>ABOUT ME</Link>
          </li>
          <li className='flex flex-row items-center border-2 border-zinc-500 rounded-full px-2 bg-transparent cursor-pointer hover:bg-zinc-600 hover:duration-300 ease-out'>
            <span className='p-2' onClick={openResume}>RESUME</span>
        ...
        <ul className={`fixed left-0 top-0 w-3/5 h-screen bg-zinc-950 bg-opacity-95 ease-out duration-500 ${nav ? '' : 'left-[-60%]'}`}>
